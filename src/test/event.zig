...
test "simple" {
    ...
    const events_to_process = [_]evt.Event{
        .{
    ...
    var events: std.ArrayList(evt.Event) = .empty;
...

    {
        ...
        for (0..event_strs.items.len) |i| {
            ...
            const event = try std.json.parseFromSliceLeaky(evt.Event, arena.allocator(), event_str, .{});
    ...
    const Ctx = struct {
        ...
        pub fn run(ctx: @This(), cursor: *Repo.DB.Cursor(.read_write)) !void {
            ...
            var last_event_id_maybe: ?[evt.id_size]u8 = null;
            if (try haxy.getCursor(hash.hashInt(repo_opts.hash, "last-event-id"))) |last_event_id_cursor| {
                var last_event_id_buffer: [evt.id_size]u8 = undefined;
            ...
            for (ctx.events) |event| {
                ...
                var current_event_id_buffer: [evt.id_size]u8 = undefined;
